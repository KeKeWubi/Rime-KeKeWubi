...
* 按 Ctrl+0 切换三种五笔方案（可可五笔 - 86版、可可五笔 - 98版、可可五笔 - 新世纪版）。
* 用户修改方案文件 keke_wubi_XX.schema.yaml 这一个文件里的配置项即可实现不同的功能，能修改的地方都加了备注，修改完要重新部署。
* 重要提醒：没加备注的配置项，会产生一些乱七八糟的功能，建议不要修改。
...
* 关于性能：可可追求极致。以万能z模糊查询为例，没有这个功能，可可使用内存6M左右；增加这个功能但不使用（“禁用万能Z”状态）内存使用8M，使用这个功能但没有按z键时内存12M，
  ...
  main版本都包含万能z键，但默认“禁用”，需要这个功能按 Ctrl+0 打开（手机端有开关），注意：打开此功能，电脑只对当前应用程序有效，苹果手机“只对本次文本编辑区域”有效。
  万能Z键“支持”五笔反查。
* 文件结构及主要功能解释如下：
...
├─keke_wubi_update_MacOS.sh				#可可五笔Macos自动更新脚本
