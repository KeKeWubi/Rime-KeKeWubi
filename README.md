...
* 关于性能：可可追求极致。以万能z为例，没有这个功能，可可使用内存6M左右；增加这个功能但不使用（关闭状态）内存使用8M，使用这个功能但没有按z键时内存11M左右，
  只有在有z键时瞬间到达32m，这足以使一个有洁癖的技术员心痛不已。为解决这个问题，特提供一个“无万能z键”的分支，但不再维护。此后版本都有这个功能，但默认关闭。
* 文件结构及主要功能解释如下：
